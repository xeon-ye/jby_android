...
/**
 * Created by AnYng on 2017/9/4 14:16.
 * E-mail: 18055070998@163.com
 * <p>
 * 流程控制的比较严谨，比如setup函数的使用
 * updateShaderMatrix保证图片损失度最小和始终绘制图片正中央的那部分
 * 作者思路是画圆用渲染器位图填充，而不是把Bitmap重绘切割成一个圆形图片。
 */
public class AwViewCircleImage extends AppCompatImageView {
    ...
    //构造函数
    public AwViewCircleImage(Context context) {
        super(context);
        init();
    }

    //构造函数
    public AwViewCircleImage(Context context, AttributeSet attrs) {
     ...
     * 构造函数
     */
    public AwViewCircleImage(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.AwViewCircleImage, defStyle, 0);
        //通过TypedArray提供的一系列方法getXXXX取得我们在xml里定义的参数值；
        // 获取边界的宽度
        mBorderWidth = a.getDimensionPixelSize(R.styleable.AwViewCircleImage_border_width, DEFAULT_BORDER_WIDTH);
        // 获取边界的颜色
        mBorderColor = a.getColor(R.styleable.AwViewCircleImage_border_color, DEFAULT_BORDER_COLOR);
        mBorderOverlay = a.getBoolean(R.styleable.AwViewCircleImage_border_overlay, DEFAULT_BORDER_OVERLAY);
