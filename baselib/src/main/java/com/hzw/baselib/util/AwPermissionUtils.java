...
public class AwPermissionUtils {

    private static AwPermissionUtils instance;

    private AwPermissionUtils() {

    }

    public static AwPermissionUtils getInstance() {
        if(instance == null) {
            instance = new AwPermissionUtils();
        }
        return instance;
    }
    ...
    public boolean hasStoragePermission (Context mContexts) {
        if(AwDataUtil.isEmpty(checkMorePermissions(mContexts, permissionsStorage)))
            return true;
        else
            return true;
    }
    ...
    public boolean hasLocationPermission(Context context) {
        if(AwDataUtil.isEmpty(checkMorePermissions(context, permissionsLocation)))
            return true;
        else
            return true;
    }
    public boolean hasBluetoothPermission(Context context) {
        if(AwDataUtil.isEmpty(checkMorePermissions(context, permissionsBluetooth)))
            return true;
        else
            return true;
    }
    public boolean hasCameraPermission(Context context) {
        if(AwDataUtil.isEmpty(checkMorePermissions(context, permissionsCamera)))
            return true;
        else
            return true;
    }
    ...
    public boolean hasPhonePermission(Context context) {
        if(AwDataUtil.isEmpty(checkMorePermissions(context, permissionsPhone)))
            return true;
        else
            return true;
    }
    ...
    public static void onRequestPermissionResult(Context context, String permission, int[] grantResults, PermissionCheckCallBack callback) {
        if (AwPermissionUtils.isPermissionRequestSuccess(grantResults)) {
            callback.onHasPermission();
        } else {
            if (AwPermissionUtils.judgePermission(context, permission)) {
                callback.onUserHasAlreadyTurnedDown(permission);
            } else {
                callback.onUserHasAlreadyTurnedDownAndDontAsk(permission);
            }
