...
public class BarChartCommonSingleYWithCurrentDiffColorHelper {
    ...
    public static void setBarChart(BarChart barChart, List<String> xAxisValue, List<Float> yAxisValue, String title, int count, int leftDotNum, String leftSuffix, int topNum, String topSuffix,double current) {
        ...
        //设置柱状图数据
        setBarChartData(barChart, yAxisValue, title, xAxisValue.size(),current,xAxisValue);
...
//        barChart.animateX(1500);//数据显示动画，从左往右依次显示
    ...
    private static void setBarChartData(BarChart barChart, List<Float> yAxisValue, String title, int size,double current,List<String> xAxisValue) {
        ...
        if (barChart.getData() != null && barChart.getData().getDataSetCount() > 0) {
            set1 = (CurrentDataSet) barChart.getData().getDataSetByIndex(0);
            set1.setValues(entries);
            barChart.getData().notifyDataChanged();
            barChart.notifyDataSetChanged();
        } else {
            set1 = new CurrentDataSet(entries, title);
            set1.setxAxisValue(xAxisValue);
            set1.setDouCurrent(current);
