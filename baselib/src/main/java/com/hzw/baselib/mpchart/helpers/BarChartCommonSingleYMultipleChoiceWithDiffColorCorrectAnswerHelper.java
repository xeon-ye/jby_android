...
//        barChart.animateX(1500);//数据显示动画，从左往右依次显示
    ...
    private static void setBarChartData(BarChart barChart, List<Float> yAxisValue, String title, int correctAnswerIndex, int size) {
        ...
        MultipleBarDataSetWithCorrectAnswer set1;

        if (barChart.getData() != null && barChart.getData().getDataSetCount() > 0) {
            set1 = (MultipleBarDataSetWithCorrectAnswer) barChart.getData().getDataSetByIndex(0);
            set1.setValues(entries);
            barChart.getData().notifyDataChanged();
            barChart.notifyDataSetChanged();
        } else {
            set1 = new MultipleBarDataSetWithCorrectAnswer(entries, title);
            set1.setColors(new int[]{Color.parseColor("#669900"),Color.parseColor("#F19F10"),Color.parseColor("#FF0000")});
