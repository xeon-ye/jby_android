...
import com.hzw.baselib.mpchart.MyBarDataSetWithCorrectAnswer;
import com.hzw.baselib.mpchart.SingleChoiceBarDataSet;
...
public class BarChartCommonSingleYWithDiffColorCorrectAnswerHelper {
    ...
    public static void setBarChart(BarChart barChart, List<String> xAxisValue, List<Float> yAxisValue, String title, int count, int leftDotNum, String leftSuffix, int topNum, String topSuffix, int correctAnswerIndex) {
        ...
        //设置柱状图数据
        setBarChartData(barChart, yAxisValue, title, correctAnswerIndex, xAxisValue.size());
...
//        barChart.animateX(1500);//数据显示动画，从左往右依次显示
    ...
    public static void setBarChartByAnswer(BarChart barChart, List<String> xAxisValue, List<Float> yAxisValue, String title, int count, int leftDotNum, String leftSuffix, int topNum, String topSuffix, int correctAnswerIndex,String answer) {
        ...
        //设置柱状图数据
        setBarChartDataByAnswer(barChart, yAxisValue, title, correctAnswerIndex, xAxisValue.size(),answer,xAxisValue);
        barChart.setScaleEnabled(false);//设置能否缩放
        barChart.setFitBars(true);//使两侧的柱图完全显示
...
//        barChart.animateX(1500);//数据显示动画，从左往右依次显示
    ...
    private static void setBarChartData(BarChart barChart, List<Float> yAxisValue, String title, int correctAnswerIndex, int size) {
        ...
        MyBarDataSetWithCorrectAnswer set1;

        if (barChart.getData() != null && barChart.getData().getDataSetCount() > 0) {
            set1 = (MyBarDataSetWithCorrectAnswer) barChart.getData().getDataSetByIndex(0);
            set1.setValues(entries);
            barChart.getData().notifyDataChanged();
            barChart.notifyDataSetChanged();
        } else {
            set1 = new MyBarDataSetWithCorrectAnswer(entries, title, correctAnswerIndex);
            set1.setColors(new int[]{Color.parseColor("#669900"),Color.parseColor("#40A0FF")});
            ...
            data.setValueFormatter(new CommonStrFormatter(0, AwBaseConstant.COMMON_SUFFIX_PERSON));
            barChart.setData(data);
       }
    }

    private static void setBarChartDataByAnswer(BarChart barChart, List<Float> yAxisValue, String title, int correctAnswerIndex, int size,String answer,List<String> xAxisValue) {
        ...
        SingleChoiceBarDataSet set1;

        if (barChart.getData() != null && barChart.getData().getDataSetCount() > 0) {
            set1 = (SingleChoiceBarDataSet) barChart.getData().getDataSetByIndex(0);
            set1.setValues(entries);
            barChart.getData().notifyDataChanged();
            barChart.notifyDataSetChanged();
        } else {
            set1 = new SingleChoiceBarDataSet(entries, title, answer);
            set1.setColors(new int[]{Color.parseColor("#669900"),Color.parseColor("#FF0000")});
            set1.setxAxisValue(xAxisValue);
            set1.setDrawValues(true);//隐藏柱子顶部文字或数字说明
            set1.setHighlightEnabled(false);
            ArrayList<IBarDataSet> dataSets = new ArrayList<>();
            dataSets.add(set1);
