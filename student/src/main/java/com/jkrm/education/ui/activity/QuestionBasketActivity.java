...
public class QuestionBasketActivity extends AwMvpActivity<QuestionBasketActPresent> implements QuestionBasketView.View, SwipeRefreshLayout.OnRefreshListener {
    ...
    @Override
    protected QuestionBasketActPresent createPresenter() {
...
//            showMsg(getString(R.string.common_function_is_dev));
    ...
    public void getPracticeDataListSuccess(PracticeDataResultBean bean) {
        ...
        SpannableString spannableString = new SpannableString("导出（" + mParcticeQuestBeanList.size() + "）");
        spannableString.setSpan(new ForegroundColorSpan(Color.RED),3,spannableString.length()-1, Spanned.SPAN_INCLUSIVE_INCLUSIVE);

        mAdapter.addAllData(mParcticeQuestBeanList);
        mAdapter.loadMoreComplete();
        mAdapter.disableLoadMoreIfNotFullPage(mRcvData);
        if(mParcticeQuestBeanList.size()>99){
            mTvNum.setText("99");
        }else{
            mTvNum.setText(mParcticeQuestBeanList.size()+"");
        }
    }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.tv_output_new:
                ...
                AwPopupwindowUtil.showCommonListWithTitlePopupWindow(mActivity, "请选择导出内容", DataUtil.getBasketExportChoiceList(), mTvOutput, bean -> {
                    String temp = (String) bean;
                    if(temp.contains("仅")) {
                        exportBasket(ids, false);
                    } else if(temp.contains("答案")) {
