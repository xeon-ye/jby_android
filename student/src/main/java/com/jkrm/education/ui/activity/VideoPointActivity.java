...
import com.jkrm.education.mvp.presenters.VideoPointPresent;
...
public class VideoPointActivity extends AwMvpActivity<VideoPointPresent> implements VideoPointView.View {
    ...
    @BindView(R.id.tv_errorQuestionStatistics)
    TextView mTvErrorQuestionStatistics;
    ...
    @Override
    protected VideoPointPresent createPresenter() {
        return new VideoPointPresent(this);
    }
    ...
    protected void initData() {
        ...
        if(!AwDataUtil.isEmpty(templateId)) {
            showView(mTvErrorQuestionStatistics, false); //20191106 需求更改, 都不显示了, 原有该处应该显示错题入口
            mPresenter.templateInfo(templateId);
        } else {
            showView(mTvErrorQuestionStatistics, false);
            showView(mTvCourseTitle, false);
        }

//        String homeworkName = getIntent().getStringExtra(Extras.COMMON_PARAMS);
//        setText(mTvCourseTitle, homeworkName);
    ...
    protected void initListener() {
        ...
        mIvVideoBack.setOnClickListener(v -> finish());
        mTvErrorQuestionStatistics.setOnClickListener(v -> toClass(ErrorQuestionStatisticsActivity.class, false, Extras.KEY_TEMPLATE_ID, templateId));
    }
    ...
    @Override
    public void templateInfoSuccess(TemplateInfoResultBean result) {
        if(result != null)
            setText(mTvCourseTitle, result.getName());
    }
}
