 ...
 * @Author: xiangqian
 * @CreateDate: 2020/3/26 18:23
 */

public class RepaidBalancePresent extends AwCommonPresenter implements RepaidBalanceView.Presenter {
    private RepaidBalanceView.View mView;

    public RepaidBalancePresent(RepaidBalanceView.View view) {
        mView = view;
    }
    ...
    @Override
    public void getInvestList(String clientType) {
        Observable<ResponseBean<List<InvestResultBean>>> observable = RetrofitClient.builderRetrofit()
                .create(APIService.class)
                .getInvestList(clientType);
        addIOSubscription(observable, new AwApiSubscriber(new AwApiCallback<List<InvestResultBean>>() {
            @Override
            public void onStart() {
            ...
            @Override
            public void onSuccess(List<InvestResultBean> data) {
                mView.getInvestListSuccess(data);
            }
