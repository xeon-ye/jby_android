 ...
 * @Author: xiangqian
 * @CreateDate: 2020/3/18 18:24
 */

public class CourseInProAdapter extends BaseQuickAdapter<DaoMicroLessonBean, BaseViewHolder> {
    private List<DaoMicroLessonBean> mList = new ArrayList<>();
    private boolean isChose;
    private CheckBox mChoseAll;

    public CourseInProAdapter() {
    ...
    protected void convert(BaseViewHolder helper, DaoMicroLessonBean item) {
        helper.addOnClickListener(R.id.iv_img)
                ...
                .setVisible(R.id.iv_img_center, true);
        ...
        if (item.getIsPause()) {
            Glide.with(mContext).load(R.mipmap.course_cache_inpro_down).into(iv_img_center);
        } else {
            Glide.with(mContext).load(R.mipmap.course_cache_inpro_pause).into(iv_img_center);
        }
...
*/
    ...
    private boolean checkIsAllChose(List<DaoMicroLessonBean> courseSuccessBeanArrayList) {
        boolean allChose = true;
        for (DaoMicroLessonBean courseSuccessBean : courseSuccessBeanArrayList) {
