...
public class RichTextMessageHolder extends MessageHolderBase implements View.OnClickListener {
    ...
    private View sobot_ll_switch;//换一组按钮
    ...
    public void bindData(final Context context, final ZhiChiMessageBase message) {
        ...
        if (message.isGuideGroupFlag() && message.getListSuggestions() != null
                && message.getListSuggestions().size() > 0
                && message.getGuideGroupNum() < message.getListSuggestions().size()) {
    ...
    public void onClick(View v) {
        if (v == sobot_ll_switch) {
            ...
            if (message != null && message.getListSuggestions() != null
                ...
                int a =8;
                int b = 5;
//                int maxNum = (int) Math.ceil(message.getListSuggestions().size() / ((double) message.getGuideGroupNum()));
                int maxNum = (int) Math.ceil(a / ((double) b));
                pageNum = pageNum > maxNum ? maxNum : pageNum;
                message.setGuideGroupNum(pageNum);
                resetAnswersList();
            }
        }
    }
