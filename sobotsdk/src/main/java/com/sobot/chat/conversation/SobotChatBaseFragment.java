...
//			e.printStackTrace();
     ...
     * @param groupName
     */
    protected void requestQueryFrom(final String groupId, final String groupName, final int transferType){
    ...
    public void remindRobotMessage(final Handler handler, final ZhiChiInitModeBase initModel,final Information info) {
        ...
        if (remindRobotMessageTimes == 1) {
            if (initModel.getUstatus() == ZhiChiConstant.ustatus_robot) {
    ...
    protected void processNewTicketMsg(final Handler handler) {
        if (initModel.getMsgFlag() == ZhiChiConstant.sobot_msg_flag_open
            ...
            zhiChiApi.checkUserTicketInfo(SobotChatBaseFragment.this, initModel.getUid(), initModel.getCompanyId(), initModel.getCustomerId(), new StringResultCallBack<SobotUserTicketInfoFlag>() {
                @Override
                public void onSuccess(SobotUserTicketInfoFlag data) {
                    if (data.isExistFlag()) {
                        ...
                        reply1.setMsg("<a href='sobot:SobotTicketInfo'>"+getResString("sobot_new_ticket_info")+"</a>");
